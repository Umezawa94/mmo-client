...
class Game {
    ...
    createScene() {
        ...
        let sphere = BABYLON.MeshBuilder.CreateSphere('sphere1', { segments: 16, diameter: 200 }, this._scene);
        ...
        ground = BABYLON.MeshBuilder.CreateGround('ground1', { width: 100, height: 100, subdivisions: 2 }, this._scene);
        ...
        let pipeline = this._pipeline = new BABYLON.DefaultRenderingPipeline("default", // The name of the pipeline
    ...
    cellify(mesh) {
        ...
        for (let i = 0; i < materials.length; i++) {
            materials[i] = this.replaceCellMat(materials[i]);
        }
    }
    replaceCellMat(mat) {
    ...
    doRender() {
        ...
        this._engine.runRenderLoop(() => {
            ...
            function intensity(time, exp = 128, sunset = 0.51) {
                time %= 2;
                if (time < sunset)
