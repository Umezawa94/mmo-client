...
export class SkySystem extends System{
    ...
    private _starboxMaterial! : BABYLON.PushMaterial;
    ...
    private _sunSpeed : number = 1/36000;

    private _lensFlareSource! : BABYLON.AbstractMesh;
    private _lensFlareSystem! : LensFlareSystem;

    constructor(scene:BABYLON.Scene, pipeline : BABYLON.DefaultRenderingPipeline){
        super(scene);
    ...
    protected update(scene : BABYLON.Scene, delta : number){
    ...
    protected lazyUpdate(scene : BABYLON.Scene){
        ...
        (<BABYLON.BackgroundMaterial>this._starboxMaterial).reflectionTexture!.level = (1 - int128)*2;
    ...
    private intensity(time:number, exp:number = 128, sunset:number = 0.51) : number{
    ...
    public get sunSpeed() : number{
        return this._sunSpeed;
    }
    public set sunSpeed(value:number){
